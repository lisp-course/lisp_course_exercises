;;; This is the object world we use for assignment 4.
;;; Also this is the standard solution of the oop-world from assignment 3.
...
(defclass entity ()
  ((coordinate :type coordinate
   ...
   (name :type symbol
         ...
         :initarg :name
         :accessor name))
  (:documentation "A standard object in the world, containing its coordinate, name and world."))
...
Also launches the visualization."
  ...
  (let* ((world (make-instance 'treasure-world))
         ...
         (scene (if random-scene
                    ...
                    scene0)))
    (mapcar (lambda (xy)
              (add-object-to-world 'wall world
                                   (intern (format nil "WALL~a-~a" (first xy) (second xy)))
                                   (make-coordinate :x (first xy) :y (second xy))))
            +wall-coords+)
    ;; (add-object-to-world 'depot world :RED
    ;;                      (make-coordinate :x (first (first (first scene)))
    ;;                                       :y (second (first (first scene))))
    ;;                      :color :RED)
    ;; (add-object-to-world 'depot world :BLUE
    ;;                      (make-coordinate :x (first (second (first scene)))
    ;;                                       :y (second (second (first scene))))
    ;;                      :color :BLUE)
    (dotimes (i 10 t)
      (funcall (lambda (xy)
...
(defun test-me-cluttered ()
  ...
  (btr-wrapper::init-world)
  (let* ((world (make-instance 'treasure-world))
         (wall-coords (append (alexandria:map-product 'list '(0) (alexandria:iota 15))
                              (alexandria:map-product 'list '(14) (alexandria:iota 14 :start 1))
                              (alexandria:map-product 'list (alexandria:iota 15) '(15))
                              (alexandria:map-product 'list (alexandria:iota 15) '(0))
                              '((1 1) (1 7) (1 11)
