...
(defun draw-circle ()
  (loop do
    (send-vel-cmd 3 (/ pi 5))
    (wait-duration 0.1)))
;; To stop the endless loop press Ctrl-C Ctrl-C
...
(cpl:top-level
  (cpl:pursue
    (unwind-protect
         (cpl:wait-for (cpl:>
                        (cpl:fl-funcall #'turtlesim-msg:x
                                        *turtle-pose*)
                        5.0))
      (send-vel-cmd 0 0))
    (draw-circle)))
...
;; (send-vel-cmd 0 0)
;; will be executed immediately, when wait-for terminates,
...
;; It has to be a child of CPL:SIMPLE-PLAN-FAILURE

;; And a function that checks the condition for fluents
(defun out-of-bounds (fluent-position)
  (roslisp:with-fields (turtlesim-msg:x turtlesim-msg:y)
      (cpl:value fluent-position)
    (not (and (< 0.5 turtlesim-msg:x 10.5)
              (< 0.5 turtlesim-msg:y 10.5)))))

;; With the following we will throw an error without handling it
;; seq is alias for progn
(cpl:top-level
  (cpl:pursue
    (cpl:seq
      (cpl:wait-for (cpl:fl-funcall #'out-of-bounds *turtle-pose*))
...
;; execute plans in order with try-in-order.
(cpl:top-level
  (cpl:try-in-order
    (cpl:seq
      (print "first one")
