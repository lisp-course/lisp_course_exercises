;;; This is the object world we use for extra assignment.
...
Also launches the visualization."
  ...
  (let* ((world (make-instance 'treasure-world))
    ...
    ;; What kind of stupid fuck is LISP doing with array?!?
    ;; Keeping the old reference? REALLY?!? Need to reset this manually.
    ;; REALLY?!?!?!
    (setf (aref (trunk (robot world)) 0) nil)
    (setf (aref (trunk (robot world)) 1) nil)
    (dotimes (i 10 t)
      (funcall (lambda (xy)
